...
void mesh_vendor_specific_data(uint16_t src, uint16_t company_id, uint16_t model_id, uint8_t opcode, void *p_data, uint16_t data_len)
{
    mesh_provision_cb_t *p_cb = &provision_cb;
    ...
    p_cb->p_vendor_specific_data(p_name, company_id, model_id, opcode, p_data, data_len);
}
...
//    case WICED_BT_MESH_LIGHT_LIGHTNESS_LINEAR_STATUS:
    ...
    case WICED_BT_MESH_VENDOR_DATA:
        mesh_vendor_specific_data(p_event->src, p_event->company_id, p_event->model_id, (uint8_t)p_event->opcode, p_data, p_event->data_len);
...
void mesh_provision_state_provisioning(mesh_provision_cb_t *p_cb, uint16_t event, wiced_bt_mesh_event_t *p_event, void *p_data)
{
    ...
    case WICED_BT_MESH_PROVISION_GET_OOB_DATA:
        sprintf(buf, "Provision: get OOB from Server:%x type:%d size:%d action:%d",
...
int mesh_client_key_refresh_phase1_continue(mesh_provision_cb_t *p_cb, wiced_bt_mesh_db_net_key_t *net_key)
{
    ...
    Log("KR phase1 continute key phase:%d\n", net_key->phase);
...
void configure_queue_remote_device_operations(mesh_provision_cb_t *p_cb)
{
    ...
    wiced_bt_mesh_db_app_key_t *app_key, *app_key_setup, *p_app_key;
...
{
    wiced_bt_mesh_proxy_filter_change_addr_data_t filter_addr;
    wiced_bt_mesh_event_t *p_event = mesh_configure_create_event(0xFFFF, WICED_FALSE);
    if (p_event != NULL)
    {
        filter_addr.addr_num = 1;
        filter_addr.addr[0] = addr;

        Log("Filter add addr:%04x num:%d first addr:%04x", p_event->dst, filter_addr.addr_num, filter_addr.addr[0]);
        return wiced_bt_mesh_proxy_filter_change_addr(p_event, WICED_TRUE, &filter_addr);
    }
    return WICED_FALSE;
}
...
int mesh_client_listen_for_app_group_broadcasts(char *control_method, char *group_name, wiced_bool_t start_listen)
{
    ...
    for (p_groups = groups_array; *p_groups != 0; p_groups += (strlen(p_groups) + 1))
    {
        ...
        if ((p_op = (pending_operation_t *)wiced_bt_get_buffer(sizeof(pending_operation_t))) != NULL)
        {
            p_op->operation = (uint8_t) (start_listen ? CONFIG_OPERATION_FILTER_ADD : CONFIG_OPERATION_FILTER_DEL);
            p_op->p_event = mesh_configure_create_event(0xFFFF, WICED_FALSE);
            p_op->uu.filter_add.addr_num = 1;
            p_op->uu.filter_add.addr[0] = group_addr;
            configure_pending_operation_queue(p_cb, p_op);
        }
