...
uint8_t *wiced_bt_mesh_format_hci_header(uint16_t dst, uint16_t app_key_idx, uint8_t element_idx, uint8_t reliable, uint8_t send_segmented, uint8_t retransmit_count, uint8_t retransmit_interval, uint8_t reply_timeout, uint16_t num_in_group, uint16_t *group_list, uint8_t *p_buffer, uint16_t len);
...
void process_properties_status(uint8_t *p_buffer, uint16_t len)
{
    ...
    p_buffer++;
    for (int i = 0; i < p_data->properties_num; i++)
    {
...
/*
 * The application can call this function to get the detailed status of the current firmware distribution process.
 */
wiced_bool_t wiced_bt_mesh_fw_provider_get_detailed_status(wiced_bt_mesh_event_t *p_event)
{
    return WICED_FALSE;
}
...
/*
 * The application can call this function to get the stuts information about every node that is participating in the current firmware distribution process.
 */
wiced_result_t wiced_bt_mesh_model_fw_provider_details_list_get(wiced_bt_mesh_event_t *p_event)
{
    return WICED_BT_SUCCESS;
}
...
uint8_t *wiced_bt_mesh_hci_header_from_event(wiced_bt_mesh_event_t *p_event, uint8_t *p_buffer, uint16_t len)
{
    return wiced_bt_mesh_format_hci_header(p_event->dst, p_event->app_key_idx, p_event->element_idx, p_event->reply, p_event->send_segmented, p_event->retrans_cnt, p_event->retrans_time, p_event->reply_timeout, p_event->num_in_group, p_event->group_list, p_buffer, len);
}
